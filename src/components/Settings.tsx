...
function Settings() {
  ...
  const [healthModal, setHealthModal] = useState<"closed" | "choose" | "paste">(
    "closed",
  );
  ...
  const handleExportAll = useCallback(() => {
    handleExportData({
      ...
      ...(healthWeightData.length > 0
        ? { health_weight: healthWeightData }
        : {}),
    });
  }, [
    handleExportData,
    ...
    async (xml: string, sourceLabel: string) => {
      try {
        ...
        for (let index = 0; index < observations.length; index += 1) {
          ...
          if (
            ...
            const extractPercent =
              36 +
              Math.round(((index + 1) / Math.max(observations.length, 1)) * 54);
            setHealthImport({
              status: "working",
        ...
        setHealthImport({
          status: "done",
          label: sorted.length
            ? `Imported ${sorted.length} weight records`
        ...
        if (sorted.length) {
          notifyLarge(
            "Apple Health imported",
            `${sorted.length} weight records saved.`,
          );
        } else {
          notifyLarge(
            "No weight records found",
            `Nothing to import from this ${sourceLabel}.`,
          );
        }
      } catch (err) {
        ...
        notifyError(
          ...
          `Could not import this ${sourceLabel}.`,
        );
      }
    },
      ...
      reader.onprogress = (progressEvent) => {
        if (progressEvent.lengthComputable) {
          const readPercent = Math.round(
            (progressEvent.loaded / progressEvent.total) * 24,
          );
          setHealthImport((prev) => ({
            ...prev,
            status: "working",
      ...
      <Box sx={{ width: "100%", maxWidth: 420, mt: 3 }}>
        ...
        >
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography sx={{ margin: "0 0 10px", fontSize: 16 }}>
              Default weight:
            </Typography>
            <NumberField.Root
      ...
      >
          ...
          <Button
            onClick={async () => {
              const ok = await confirm(
                "⚠️ Import will replace all current data with this backup.\n\nWARNING: This action is NOT SAFE and may break the app or cause data loss. Proceed only if you know what you are doing.\n\nContinue?",
              );
        ...
        {healthImport.status !== "idle" && (
          ...
          >
            ...
            >
              {healthImport.label}
              {healthImport.status === "working"
                ? ` · ${healthImport.percent}%`
                : ""}
            </Typography>
            <LinearProgress
              variant="determinate"
          ...
          {healthModal === "choose" ? (
            ...
            <>
              ...
              <Box sx={{ display: "flex", gap: 1 }}>
                ...
                <Button
                  variant="contained"
                  onClick={handleImportAppleHealthText}
                  sx={{ flex: 1 }}
                >
                  Import
                </Button>
